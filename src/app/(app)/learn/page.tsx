    ...
    <div className="max-w-4xl mx-auto">
      <Accordion type="single" collapsible className="w-full space-y-4" defaultValue="item-0">
        {learningPaths.map((path, index) => {
          const completedLessonsCount = path.lessons.filter(l => isLessonCompleted(l.slug)).length;
          const progress = Math.round((completedLessonsCount / path.lessons.length) * 100);
              ...
              <AccordionTrigger className="bg-card p-6 hover:bg-accent/50 data-[state=open]:rounded-b-none transition-all text-left">
                <div className="flex-1">
                  <div className="flex justify-between items-center">
                    ...
                    <Badge variant={progress > 0 ? "secondary" : "outline"} className="text-secondary-foreground">{progress}% Complete</Badge>
                  </div>
